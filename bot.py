...
def sm4_lt(ka):
    b = sm4_sbox(ka)
    return (b ^ rotl(b,2) ^ rotl(b,10) ^ rotl(b,18) ^ rotl(b,24)) & 0xffffffff

def sm4_calci_rk(ka):
    b = sm4_sbox(ka)
    return (b ^ rotl(b,13) ^ rotl(b,23)) & 0xffffffff

def sm4_f(x0, x1, x2, x3, rk):
    return (x0 ^ sm4_lt(x1 ^ x2 ^ x3 ^ rk)) & 0xffffffff
...
def sm4_encrypt_ecb(pt):
    ...
    k = [0]*36
    for i in range(4):
        k[i] = (mk[i] ^ FK[i]) & 0xffffffff
    sk = [0]*32
    for i in range(32):
        k[i+4] = (k[i] ^ sm4_calci_rk(k[i+1] ^ k[i+2] ^ k[i+3] ^ CK[i])) & 0xffffffff
        sk[i] = k[i+4]
    res = bytearray()
...
def gx_send_sms(phone, captcha_code, uuid):
    try:
        ...
        headers = HEADERS_GX.copy()
        headers["Content-Type"] = "application/x-www-form-urlencoded; charset=UTF-8"
        headers["Referer"] = "http://www.gxdlys.com/Wechat/User/Regist"
