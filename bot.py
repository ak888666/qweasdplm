        ...
        try:
            ...
            if r.status_code==200:
                ...
                if d.get("statusCode")==200:
                    uuid=d["data"].get("uuid"); img=d["data"].get("img")
                    if uuid and img: return True, img, uuid
        except: pass
        time.sleep(1)
...
def gxquery_start(update, context):
    ...
    if success:
        update.message.reply_text("⏳ 登录成功，获取验证码...")
...
def recharge_amount(update, context):
    ...
    except: update.message.reply_text("❌ 请输入数字"); return RECHARGE_AMOUNT
    if amt<=0: update.message.reply_text("金额需大于0"); return RECHARGE_AMOUNT
