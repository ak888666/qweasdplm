...
def download_photo(file_id):
    ...
    return None
async def run_full_flow(real_name,id_card):
    ...
    if ok:
        ...
        if result and result.get("statusCode")==200:
            ...
            return {"success":True,"info":info,"photo":photo_bytes}
        else: return {"success":False,"error":"查询照片失败"}
    else:
        if "未注册" in msg or "不存在" in msg:
            token=sms_login()
            if not token: return {"success":False,"error":"接码登录失败"}
            phone=get_phone(token)
            if not phone: return {"success":False,"error":"获取手机号失败"}
            captcha,uuid=get_captcha()
            if not captcha: return {"success":False,"error":"验证码识别失败"}
            if not send_sms(phone,captcha,uuid): return {"success":False,"error":"短信发送失败"}
            sms_code=get_sms(token,phone)
            if not sms_code: return {"success":False,"error":"短信验证码获取失败"}
            if not register(phone,sms_code,captcha,real_name,id_card): return {"success":False,"error":"注册失败"}
            ok2,_=login(id_card)
            if ok2:
                result=query_id_photo(real_name,id_card)
                if result and result.get("statusCode")==200:
                    ...
                    photo_bytes=download_photo(data.get("item1")) if data.get("item1") else None
                    return {"success":True,"info":info,"photo":photo_bytes}
                else: return {"success":False,"error":"注册后查询失败"}
            else: return {"success":False,"error":"注册后登录失败"}
        else: return {"success":False,"error":f"登录失败：{msg}"}
...
async def process_and_reply(update,context,real_name,id_card):
    try:
        result=await run_full_flow(real_name,id_card)
        if result['success']:
            await context.bot.send_message(chat_id=update.effective_chat.id,text=f"✅ 成功！\n{result['info']}")
            if result.get('photo'):
                await context.bot.send_photo(chat_id=update.effective_chat.id,photo=io.BytesIO(result['photo']))
        else:
            await context.bot.send_message(chat_id=update.effective_chat.id,text=f"❌ {result['error']}")
