...
# ========== 配置(必须全部替换为实际值,且只含 ASCII 字符)==========
BOT_TOKEN = "5849383582:AAHIfKvl2O3buRgiIq4rwtC4b95KsP3BfS4"   # 您的 Bot Token

FIXED_NAME = "刘德华"
SAVE_FOLDER = "temp_files"           # 改为临时目录
RETRY_TIMES = 5

# ⚠️ 以下所有值必须替换为从浏览器复制的真实值,且不能包含中文!
BASE_COOKIES = {
    "cna": "REPLACE_CNA_HERE",          # 替换为实际值
...
def query_id_card(id_card):
    """
    查询并返回 (成功标志, 数据或错误消息)
    成功时 data 为 PDF 文件的二进制内容，失败时为错误字符串
    """
        ...
        if result1.get("code") == "1":
            try:
                ...
                if res2.status_code == 200:
                    return True, res2.content   # 直接返回二进制数据
                else:
